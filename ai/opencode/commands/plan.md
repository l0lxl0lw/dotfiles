...
`$ARGUMENTS` as a GitHub issue and optional exact Research comment. The complete
upstream prompt below defines planning depth, but GitHub replaces local `thoughts/`
files: set Planning, read the issue and comments, inspect current code, and publish
an Implementation plan comment containing the source commit, research link,
approach, affected files, ordered steps, acceptance criteria, automated checks,
and manual verification. Identify superseded plans. Set Ready when complete,
return the exact comment URL, and stop without implementing.
