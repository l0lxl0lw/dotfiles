...
`$ARGUMENTS` as a GitHub issue reference. The full upstream prompt below defines
the research method, but GitHub replaces its local `thoughts/` storage: read the
issue and all comments, set Researching unless later work is active, and publish
the result as a Research comment with the investigated commit, file/line evidence,
patterns, open questions, and ticket implications. Return the comment URL. Do not
modify application code or advance to planning. No Agentic CLI is required.
...
Collect the current timestamp and run `git rev-parse HEAD`,
`git branch --show-current`, and `basename "$(git rev-parse --show-toplevel)"`.
