...
Read `~/dotfiles/ai/opencode/tracking/WORKFLOW.md` and follow its contract. Treat
`$ARGUMENTS` as a GitHub issue and optional exact plan-comment URL. The complete
upstream prompt below defines implementation discipline, but the approved plan is
an issue comment rather than a `thoughts/` file. Confirm the plan, repository, and
isolated feature branch/worktree; register the branch and refresh freshness. Ask
before explicit sync when needed. Set Implementing, post a start comment, implement
and verify the plan, then post progress with tests and remaining checks. Git actions
beyond the implementation require the user's normal explicit authorization. Do not
mark Done merely because local implementation completed.
