...
`$ARGUMENTS` as a GitHub issue and optional PR URL. The full upstream prompt below
defines review depth, but GitHub replaces local ticket, plan, and review files.
Read the issue discussion and actual branch/PR diff, refresh registered branch
freshness, and publish a Review comment with severity-ordered findings, exact
file/line references, test results, and remaining gaps. Review is read-only for
application code. Do not merge, close the issue, or mark Done because review passed.
