...
def main():
    ...
    env = launch.environment(bundle, "balanced")
    ...
    cfg["command"]["permission-smoke"] = {"description": "Disposable permission probe", "agent": "workflow-plan",
        "model": "openai/gpt-6-astra", "variant": "medium", "subtask": True, "template": "$ARGUMENTS"}
    cfg["command"]["model-smoke"] = {"description": "Resolved execution profile probe", "agent": "workflow-execute",
        "model": "openai/gpt-5.6-sol", "variant": "xhigh", "subtask": True, "template": "Reply MODEL_READY only. No tools."}
    ...
    with tempfile.TemporaryDirectory(prefix="runtime-smoke-", dir=ROOT / "tests") as tmp:
        ...
        with (directory / "server.log").open("w") as log:
            ...
            try:
                ...
                _, roles = launch.profile(bundle / "opencode", "balanced")
                ...
                providers = next(p for p in api("GET", "/provider")["all"] if p["id"] == "openai")["models"]
                for settings in roles.values():
                    assert settings["variant"] in providers[settings["model"].split("/", 1)[1]]["variants"]
                ...
                for name in ("permission-smoke", "model-smoke"):
                    ...
                    expected = ("gpt-6-astra", "medium") if name == "permission-smoke" else ("gpt-5.6-sol", "xhigh")
                    assert paid and all((m["modelID"], m.get("variant")) == expected for m in paid)
