...
├── hooks/                 # Shell hooks (statusline.sh = context progress bar; journal-stop.sh = end-of-turn learnings journal)
...
- The merge function only patches `statusLine` into `settings.json` — it does **not** register the `journal-stop.sh` `Stop` hook. That hook entry under `hooks.Stop` in `~/.claude/settings.json` is hand-maintained; it just needs to point at the symlinked `~/.claude/hooks/journal-stop.sh`. The script writes learnings to `~/Library/CloudStorage/Dropbox/sync/claude/journals/YYYY/MM/YYYY-MM-DD_HHMMSS_<host>_<slug>.md` (inline mechanism: it `block`s once to re-prompt Claude, which writes the entry, then steps aside on `stop_hook_active`).
