...
7. On Critic approval (with improvements applied): *(--interactive only)* If running with `--interactive`, use `AskUserQuestion` to present the plan with these options:
   - **Approve and execute via ralph** — proceed to implementation via ralph (sequential execution with verification)
...
9. On user approval (--interactive only):
   - **Approve and execute via ralph**: **MUST** invoke `Skill("ralph")` with the approved plan path from `.omc/plans/` as context. Do NOT implement directly. Do NOT edit source code files in the planning agent. The ralph skill handles execution.
   - **Clear context and implement**: First compact the context window, then invoke `Skill("ralph")` with the approved plan path from `.omc/plans/`.
...
- **CRITICAL — Consensus mode agent calls MUST be sequential, never parallel.** Always await the Architect agent result before issuing the Critic agent.
...
- In consensus mode with `--interactive`, on user approval **MUST** invoke `Skill("ralph")` for execution (step 9) -- never implement directly in the planning agent
</Tool_Usage>
...
- If the user says "just do it" or "skip planning", **MUST** invoke `Skill("ralph")` to transition to execution mode. Do NOT implement directly in the planning agent.
